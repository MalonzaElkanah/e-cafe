...
class DeliveryMessages extends Base_Controller{
	public function __construct(){
		parent::__construct();
		$this->loadInnerModel("delivery/deliveryMessages");
		
	}
	public function allMessages(){
		$message_index = $this->model->checkMessageIndex($_SESSION["deliverypersonid"]);
		$num = 0;
		$messageView = [];
		$messageData = [];
		...
		$this->view->innerRenderView('delivery/messages');
	}
	public function sendCustomerMessage(){
		if (isset($_POST['send_message'])) {
			$message = $_POST['send_message'];
			$recipient_id = $_POST['recipient_id'];
			$messageStatus = $this->model->sendCustomerMessage($message, $recipient_id, $_SESSION["deliverypersonid"]) ;
	...
	public function myChat(){
		if(isset($_GET['CHAT_ID'])){
			...
			$this->view->innerRenderView('delivery/chat');
		}
	}
