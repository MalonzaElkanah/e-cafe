<?php
class DeliveryMessages_Model extends Base_Model{
	public function __construct(){
		parent::__construct();
	}
	public function checkMessageIndex($r_id){
		$stmt = $this->db->query("SELECT message_index FROM delivery_people WHERE delivery_person_id = $r_id");
		$data = $stmt->fetch(PDO::FETCH_ASSOC);
		$hasData = $stmt->rowCount();
		$k =0;
		//$data['message_index'];
		if (is_null($data['message_index'])) {
			$status = "DELIVERY_PERSON";
			$columns = 'STATUS';
			$values =' \''.$status.'\'';
			$stmnt = $this->db->prepare("INSERT INTO message_controller($columns) VALUES($values);");
			$stmnt->execute();
			$newIndex = $this->db->lastInsertId();
			$stm = $this->db->prepare("UPDATE delivery_people SET message_index = $newIndex WHERE delivery_person_id = $r_id");
			$stm->execute();

			$message_index = $this->db->query("SELECT message_index FROM delivery_people WHERE delivery_person_id = $r_id")->fetchAll(PDO::FETCH_ASSOC);
